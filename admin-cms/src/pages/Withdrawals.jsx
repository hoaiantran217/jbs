...
export default function Withdrawals() {
  ...
  const fetchWithdrawals = async () => {
    ...
    try {
      ...
      // Lọc chỉ lấy withdrawals và sắp xếp theo createdAt mới nhất trước
  ...
  // Cập nhật totalPages khi filteredWithdrawals thay đổi
  useEffect(() => {
    setTotalPages(calculatedTotalPages);
    if (currentPage > calculatedTotalPages && calculatedTotalPages > 0) {
      setCurrentPage(1);
    }
  }, [filteredWithdrawals, calculatedTotalPages]);
    ...
    <div className="p-4 w-full bg-white rounded-lg shadow-md min-h-screen">
      ...
      {showBankInfo && bankInfo.length > 0 && (
        <div className="mb-6 p-4 bg-gray-50 rounded-lg">
          <h2 className="text-lg font-semibold mb-3">Thông tin ngân hàng để chuyển tiền</h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {bankInfo.map((bank) => (
              <div key={bank._id} className="bg-white p-4 rounded-lg border">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-green-600">{bank.bankName}</h3>
                  <button
      ...
      <ConfirmModal
        ...
        message={`Bạn chắc chắn muốn ${confirm.status === "approved" ? "duyệt" : "từ chối"} giao dịch rút tiền này?`}
